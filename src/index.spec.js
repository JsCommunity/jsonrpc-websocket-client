...
describe("Client", () => {
  ...
  describe("#close()", () => {
    it("returns a promise which resolve when disconnected", () => {
      return client.open().then(() => client.close());
    });
    ...
    it("aborts the connection if connecting", () => {
      ...
      return client
        .close()
        .then(() => expect(promise).rejects.toBeInstanceOf(AbortedConnection));
    });
    ...
    it("fails all waiting calls", () => {
      return client.open().then(() => {
        ...
        return client
          .close()
          .then(() => expect(promise).rejects.toBeInstanceOf(ConnectionError));
      });
    });
  });
  ...
  describe("#call()", () => {
    ...
    it("returns a promise which rejects with the error of the request", () => {
      return client
        .open()
        .then(() => client.call("error", ["an error"]))
        .then(
          () => {
